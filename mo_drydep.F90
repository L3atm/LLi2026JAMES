...
!
    ...
    !++Longlei Li
    allocate( lucfrac(pcols,nluc, begchunk:endchunk),stat=astat)
    if( astat /= 0 ) then
       write(iulog,*) 'dvel_inti: failed to allocate lucfrac; error = ',astat
       call endrun
    end if
    !--Longlei Li   
    if(do_soilw) then
    ...
    if(dycore_is('UNSTRUCTURED') ) then
       ...
       !++Longlei Li
       allocate( glacier(nlon_veg,nlat_veg), stat=astat )
       if( astat /= 0 ) then
          write(iulog,*) 'dvel_inti: failed to allocate glacier; error = ',astat
          call endrun
       end if
       !--Longlei Li
       ...
       !++Longlei Li
       ierr = pio_inq_varid( piofile, 'PCT_GLACIER', vid )
       ierr = pio_get_var( piofile, vid, glacier )
       !--Longlei Li
       ...
       call interp_map( plon, plat, nlon_veg, nlat_veg, npft_veg, lat_veg, lat_veg_edge, &
            ...
            wetland, glacier, vegetation_map, soilw_map, do_soilw )
       !--Longlei Li
  ...
  !--Longlei Li
    ...
    !++Longlei Li
    real(r8), intent(in)         :: glacier(nlon_veg,nlat_veg)
    !--Longlei Li
    ...
    !++Longlei Li
    real(r8), dimension(npft_veg+4)  :: fraction
    !--Longlei Li
    ...
    !++Longlei Li
    real(r8) :: tmp_lucfrac(plon,nluc,plat)
    !--Longlei Li
          ...
          do jj = ind_lat(j),ind_lat(j+1)
             ...
             do ii =ind_lon(i),ind_lon(i+1)
                ...
                if( nint(landmask(i_ndx,jj)) == 0 ) then
                   ...
                   !++Longlei Li
                   fraction(npft_veg+4) = fraction(npft_veg+4) + area * glacier(i_ndx,jj)
                   !--Longlei Li
                   !-----------------------------------------------------------------
    ...
    !++Longlei Li
    call scatter_field_to_chunk(1,nluc,1,plon,tmp_lucfrac,lucfrac)
    !--Longlei Li
    ...
    !++Longlei Li
    where (lucfrac < 0._r8) lucfrac = 0._r8
    where (lucfrac > 1._r8) lucfrac = 1._r8
    !--Longlei Li
