...
subroutine nucleati(  &
   ...
   wpice, weff, fhom, regm, freqmey,    &
   oso4_num, odst_num, osoot_num,       &
   odst1_num, odst3_num, call_frm_zm_in)
#else
subroutine nucleati(  &
   wbar, tair, pmid, relhum, cldn,      &
   qc, qi, ni_in, rhoair,               &
   ...
   real(r8), intent(in) :: x_fsp_accum
   real(r8), intent(in) :: x_fsp_coarse
#endif
...
#if ( defined DUSTTYPES )
   real(r8), intent(out) :: odst1_num, odst3_num
   real(r8), intent(out) :: freqmey    ! fractional occurance of mixed-phase clouds 
#endif
   ...
   if((so4_num.ge.1.0e-10_r8 .or. dst3_num.ge.1.0e-10_r8) .and. cldn.gt.0._r8) then
      ...
      if ((relhum*svp_water(tair)/svp_ice(tair)*subgrid).ge.1.2_r8) then
         if (((tc.le.0.0_r8).and.(tc.ge.-37.0_r8).and.(qc.lt.1.e-12_r8)).or.(tc.le.-37.0_r8))then
            A = -1.4938_r8 * log(dst3_num) + 12.884_r8
            B = -10.41_r8  * log(dst3_num) - 67.69_r8
            regm = A * log(wbar1) + B
            ...
            if (tc .gt. regm) then
               ...
               if(tc.lt.-40._r8 .and. wbar1.gt.1._r8) then ! exclude T<-40 & W>1m/s from hetero. nucleation
                  ...
                  !odst1_num = dst1_num * (niimm + nidep) / (soot_num + dst1_num)
                  !odst3_num = dst3_num * (niimm + nidep) / (soot_num + dst3_num)
                  odst3_num = 0._r8 !niimm + nidep
               endif
            ! homogeneous nucleation only
            !else if (tc.lt.regm-5._r8 .or. (soot_num+dst_num) < 1.0e-10_r8) then ! double check, Longlei
   ...
   if (atk_in) then  ! Atkinson et al., surface area based

      at = -1.038_r8*max(248.0_r8,tair) + 275.26_r8    ! valid between 248 and 268 K
...
!   It changes answers so needs climate validation.
   ...
   ! iceMP
      if (atk_in) then      ! use Atkinson et al.
         ns_dust_imm = exp(at)*1.e+4_r8     ! m^-2, valid for 248 to 268 K

         nimey=dst1_num * (1._r8 - exp(-1.0_r8 * dst1_sfc_to_num * ns_dust_imm * x_fsp_accum * k_fsp)) &   ! cm^-3
