...
type :: zm_aero_t
   ...
   integer :: mode_coarse7_dst_idx = -1  ! index of coarse mode
   integer :: mode_coarse8_dst_idx = -1  ! index of coarse mode
   integer :: mode_coarse9_dst_idx = -1  ! index of coarse mode
   integer :: mode_coarse10_dst_idx = -1  ! index of coarse mode
#if ( defined DUSTTYPES )
#if ( defined MODAL_AERO_4MODE )
   integer :: accum_bc_idx = -1
   integer :: accum_dust1_idx = -1
   integer :: accum_dust2_idx = -1
   integer :: accum_dust3_idx = -1
   integer :: accum_dust4_idx = -1
   integer :: accum_dust5_idx = -1
   integer :: accum_dust6_idx = -1
   integer :: accum_dust7_idx = -1
   integer :: accum_dust8_idx = -1
   ...
   integer :: accum_bc_idx = -1
   integer :: accum5_dust1_idx = -1
   integer :: accum5_dust2_idx = -1
   integer :: accum5_dust3_idx = -1
   integer :: accum5_dust4_idx = -1
   integer :: accum5_dust5_idx = -1
   integer :: accum5_dust6_idx = -1
   integer :: accum5_dust7_idx = -1
   integer :: accum5_dust8_idx = -1
...
#endif
!#if ( defined MODAL_AERO_4MODE )
   integer :: coarse_so4_idx = -1  ! index of nacl in coarse mode
#if ( defined MODAL_AERO_10MODE )
   !integer :: coarse6_so4_idx = -1  ! index of nacl in coarse mode
   integer :: coarse7_so4_idx = -1  ! index of nacl in coarse mode
   integer :: coarse8_so4_idx = -1  ! index of nacl in coarse mode
   integer :: coarse9_so4_idx = -1  ! index of nacl in coarse mode
   integer :: coarse10_so4_idx = -1  ! index of nacl in coarse mode
#endif
...
#if ( defined DUSTTYPES )
  ! variable to store output which is not required by this routine
  real(r8) :: freqmey(pcols,pver)                        ! fractional occurance of mixed-phase clouds
  real(r8) :: x_fsp_accum, x_fsp_coarse                  
  real(r8) :: dst1_sfc_to_num,  dst3_sfc_to_num          
  real(r8) :: soot_sfc, dst_sfc, &
...
#if ( defined DUSTTYPES )
  freqmey(1:pcols,1:pver) = 0._r8   ! iceMP
...
!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test00'
...
!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test01'

#if ( defined DUSTTYPES )                  
#if ( defined MODAL_AERO_4MODE )
                    dmc  = 0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust1_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust1_idx,aero%mode_coarse_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust2_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust2_idx,aero%mode_coarse_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust3_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust3_idx,aero%mode_coarse_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust4_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust4_idx,aero%mode_coarse_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust5_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust5_idx,aero%mode_coarse_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust6_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust6_idx,aero%mode_coarse_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust7_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust7_idx,aero%mode_coarse_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust8_idx,aero%mode_coarse_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse_dust8_idx,aero%mode_coarse_idx))                    

                    x_fsp_coarse = 0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_dust7_idx,aero%mode_coarse_idx) &
                                  +aero%mmrg_a(i,k,aero%coarse_dust7_idx,aero%mode_coarse_idx))/dmc
                    ...
                    if (dmc > 0._r8) then
                        dst3_num = dmc/(ssmc + dmc) *(aero%numg_a(i,k-1,aero%mode_coarse_idx)         &
...
! + Longlei Li
                    if ( dmc > 0._r8 ) then
                       dst3_sfc = dmc/(dmc+ssmc)*pi*( (aero%dgnumg(i,k,aero%mode_coarse_idx)+ &
                                  aero%dgnumg(i,k-1,aero%mode_coarse_idx))*0.5_r8  ) **2.0_r8*&
                                  exp(2.0_r8*log(aero%sigmag_coarse) **2.0_r8)&
                                  *(aero%numg_a(i,k-1,aero%mode_coarse_idx)+aero%numg_a(i,k,aero%mode_coarse_idx))*&
                                  0.5_r8*rho(i,k)*1.0e-6_r8      ! m2/cm^3
                    else
                       dst3_sfc = 0.0_r8
                    endif
                    dst3_sfc_to_num = pi*( (aero%dgnumg(i,k,aero%mode_coarse_idx)+ &
                                      aero%dgnumg(i,k-1,aero%mode_coarse_idx))*0.5_r8  )**2.0_r8*&
                                      exp(2.0_r8*log(aero%sigmag_coarse)**2.0_r8) ! m2/#, individual particle sfc

                    dst_num = dst3_num    ! dust number (#/cm^3), coarse mode only
                    dst_sfc = dst3_sfc    ! dust area (m2/cm^3)

#elif ( defined MODAL_AERO_10MODE )
!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test02'

                    dmc  = 0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust1_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust1_idx,aero%mode_coarse7_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust2_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust2_idx,aero%mode_coarse7_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust3_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust3_idx,aero%mode_coarse7_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust4_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust4_idx,aero%mode_coarse7_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust5_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust5_idx,aero%mode_coarse7_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust6_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust6_idx,aero%mode_coarse7_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust7_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust7_idx,aero%mode_coarse7_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust8_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust8_idx,aero%mode_coarse7_dst_idx))  +    &

                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust1_idx,aero%mode_coarse8_dst_idx)      &                  
                                  +aero%mmrg_a(i,k,aero%coarse8_dust1_idx,aero%mode_coarse8_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust2_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust2_idx,aero%mode_coarse8_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust3_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust3_idx,aero%mode_coarse8_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust4_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust4_idx,aero%mode_coarse8_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust5_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust5_idx,aero%mode_coarse8_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust6_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust6_idx,aero%mode_coarse8_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust7_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust7_idx,aero%mode_coarse8_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse8_dust8_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust8_idx,aero%mode_coarse8_dst_idx))  +    &

                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust1_idx,aero%mode_coarse9_dst_idx)      &                  
                                  +aero%mmrg_a(i,k,aero%coarse9_dust1_idx,aero%mode_coarse9_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust2_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust2_idx,aero%mode_coarse9_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust3_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust3_idx,aero%mode_coarse9_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust4_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust4_idx,aero%mode_coarse9_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust5_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust5_idx,aero%mode_coarse9_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust6_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust6_idx,aero%mode_coarse9_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust7_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust7_idx,aero%mode_coarse9_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse9_dust8_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust8_idx,aero%mode_coarse9_dst_idx))  +    &

                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust1_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust1_idx,aero%mode_coarse10_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust2_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust2_idx,aero%mode_coarse10_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust3_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust3_idx,aero%mode_coarse10_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust4_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust4_idx,aero%mode_coarse10_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust5_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust5_idx,aero%mode_coarse10_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust6_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust6_idx,aero%mode_coarse10_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust7_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust7_idx,aero%mode_coarse10_dst_idx))  +    &
                           0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse10_dust8_idx,aero%mode_coarse10_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust8_idx,aero%mode_coarse10_dst_idx))


!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test03'


                    x_fsp_coarse = 0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_dust7_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_dust7_idx,aero%mode_coarse7_dst_idx)           +    &
                                           aero%mmrg_a(i,k-1,aero%coarse8_dust7_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_dust7_idx,aero%mode_coarse8_dst_idx)           +    &
                                           aero%mmrg_a(i,k-1,aero%coarse9_dust7_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_dust7_idx,aero%mode_coarse9_dst_idx)           +    &      
                                           aero%mmrg_a(i,k-1,aero%coarse10_dust7_idx,aero%mode_coarse10_dst_idx)    &
                                  +aero%mmrg_a(i,k,aero%coarse10_dust7_idx,aero%mode_coarse10_dst_idx))/dmc

                    ssmc = 0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse7_so4_idx,aero%mode_coarse7_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse7_so4_idx,aero%mode_coarse7_dst_idx)        &
                                  +aero%mmrg_a(i,k-1,aero%coarse8_so4_idx,aero%mode_coarse8_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse8_so4_idx,aero%mode_coarse8_dst_idx)        &
                                  +aero%mmrg_a(i,k-1,aero%coarse9_so4_idx,aero%mode_coarse9_dst_idx)      &
                                  +aero%mmrg_a(i,k,aero%coarse9_so4_idx,aero%mode_coarse9_dst_idx)        &
                                  +aero%mmrg_a(i,k-1,aero%coarse10_so4_idx,aero%mode_coarse10_dst_idx)    &
                                  +aero%mmrg_a(i,k,aero%coarse10_so4_idx,aero%mode_coarse10_dst_idx))

!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test04'

                    if (dmc > 0._r8) then
                        dst3_num = dmc/(ssmc + dmc) *(aero%numg_a(i,k-1,aero%mode_coarse7_dst_idx)        &
                                  + aero%numg_a(i,k,aero%mode_coarse7_dst_idx)                            &
                                  + aero%numg_a(i,k-1,aero%mode_coarse8_dst_idx)                          & 
                                  + aero%numg_a(i,k,aero%mode_coarse8_dst_idx)                            &
                                  + aero%numg_a(i,k-1,aero%mode_coarse9_dst_idx)                          &  
                                  + aero%numg_a(i,k,aero%mode_coarse9_dst_idx)                            & 
                                  + aero%numg_a(i,k-1,aero%mode_coarse10_dst_idx)                         &
                                  + aero%numg_a(i,k,aero%mode_coarse10_dst_idx))*0.5_r8*rho(i,k)*1.0e-6_r8
                    else
                       dst3_num = 0.0_r8
                    end if

!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test05'


! + Longlei Li
                    if ( dmc > 0._r8 ) then
                       dst3_sfc = dmc/(dmc+ssmc)*pi*( (aero%dgnumg(i,k,aero%mode_coarse7_dst_idx)+ &
                                  aero%dgnumg(i,k-1,aero%mode_coarse7_dst_idx) + &
                                  aero%dgnumg(i,k,aero%mode_coarse8_dst_idx) + &
                                  aero%dgnumg(i,k-1,aero%mode_coarse8_dst_idx) + &
                                  aero%dgnumg(i,k,aero%mode_coarse9_dst_idx) + &
                                  aero%dgnumg(i,k-1,aero%mode_coarse9_dst_idx) + &
                                  aero%dgnumg(i,k,aero%mode_coarse10_dst_idx) + &
                                  aero%dgnumg(i,k-1,aero%mode_coarse10_dst_idx))*0.5_r8 ) **2.0_r8*&
                                  exp(2.0_r8*log(aero%sigmag_coarse) **2.0_r8)&
                                  *(aero%numg_a(i,k-1,aero%mode_coarse7_dst_idx)+aero%numg_a(i,k,aero%mode_coarse7_dst_idx) + &
                                    aero%numg_a(i,k-1,aero%mode_coarse8_dst_idx)+aero%numg_a(i,k,aero%mode_coarse8_dst_idx) + &
                                    aero%numg_a(i,k-1,aero%mode_coarse9_dst_idx)+aero%numg_a(i,k,aero%mode_coarse9_dst_idx) + &
                                    aero%numg_a(i,k-1,aero%mode_coarse10_dst_idx)+aero%numg_a(i,k,aero%mode_coarse10_dst_idx))* &
                                  0.5_r8*rho(i,k)*1.0e-6_r8      ! m2/cm^3
                    else
                       dst3_sfc = 0.0_r8
                    endif
                    dst3_sfc_to_num = pi*( (aero%dgnumg(i,k,aero%mode_coarse7_dst_idx)+ &
                                      aero%dgnumg(i,k-1,aero%mode_coarse7_dst_idx)    + &
                                      aero%dgnumg(i,k,aero%mode_coarse8_dst_idx)      + &
                                      aero%dgnumg(i,k-1,aero%mode_coarse8_dst_idx)    + &
                                      aero%dgnumg(i,k,aero%mode_coarse9_dst_idx)      + &
...
!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test06'
...
!                    ssmc = 0.5_r8*(aero%mmrg_a(i,k-1,aero%coarse_so4_idx,aero%mode_coarse_idx)      &
...
!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test07'
                    ...
                    call nucleati( &
                       ...
                       x_fsp_accum, x_fsp_coarse,                                        &
                       dum2i(i,k), nihf(i,k), niimm(i,k), nidep(i,k), nimey(i,k),        &
                       wpice, weff, fhom, temp1, freqmey(i,k), temp2, temp3, temp4,      &
                       temp31, temp33, .true. )

!   write(iulog,*) 'Longlei Li marks begin in zm_microphysics.F90: test08'
...
! Longlei Li
                    ...
                    if (dmc > 0.0_r8) then
                        nacon3 = dmc/(ssmc + dmc) * (aero%numg_a(i,k,aero%mode_coarse7_dst_idx)     &
                                 + aero%numg_a(i,k-1,aero%mode_coarse7_dst_idx)                     &
                                 + aero%numg_a(i,k,aero%mode_coarse8_dst_idx)                       &
                                 + aero%numg_a(i,k-1,aero%mode_coarse8_dst_idx)                     &
                                 + aero%numg_a(i,k,aero%mode_coarse9_dst_idx)                       &
                                 + aero%numg_a(i,k-1,aero%mode_coarse9_dst_idx)                     &
                                 + aero%numg_a(i,k,aero%mode_coarse10_dst_idx)                      &
                                 + aero%numg_a(i,k-1,aero%mode_coarse10_dst_idx))*0.5_r8*rho(i,k)
                    end if
...
!                    ssmc = 0.5_r8*(aero%mmrg_a(i,k,aero%coarse_so4_idx,aero%mode_coarse_idx)     &
!                                  +aero%mmrg_a(i,k-1,aero%coarse_so4_idx,aero%mode_coarse_idx)) 
!                    if (dmc > 0.0_r8) then
