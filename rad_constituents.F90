...
type :: mode_component_t
   ...
   character(len=  2) :: source_num_a  ! source of interstitial number conc field
   ...
   character(len=  2) :: source_num_c  ! source of cloud borne number conc field
   ...
   character(len=  2), pointer :: source_mmr_a(:)  ! source of interstitial specie mmr fields
   ...
   character(len=  2), pointer :: source_mmr_c(:)  ! source of cloud borne specie mmr fields
...
!   'fine_dust     ', 'coarse        ', 'coarse_seasalt', 'coarse1_dust   ', &
!   'coarse7_dust   ', 'coarse8_dust   ', 'coarse9_dust   ' /) ! Longlei Li MAM9

character(len=14), parameter :: mode_type_names(num_mode_types) = (/ &
   'accum         ', 'aitken        ', 'primary_carbon', 'fine_seasalt  ', &
   'fine_dust     ', 'coarse_seasalt', &
   'coarse7_dust  ', 'coarse8_dust  ', 'coarse9_dust  ', 'coarse10_dust ' /) ! Longlei Li MAM10
#if ( defined DUSTTYPES )
integer, parameter :: num_spec_types = 15
character(len=10), parameter :: spec_type_names(num_spec_types) = (/ &
   'sulfate   ', 'ammonium  ', 'nitrate   ', 'p-organic ', &
   's-organic ', 'black-c   ', 'seasalt   ', 'dust-type1', &
   'dust-type2', 'dust-type3', 'dust-type4', 'dust-type5', &
   'dust-type6', 'dust-type7', 'dust-type8'/)
#else
...
! Longlei Li
               write( iulog, '(1x,i2)' ) &
                     'Longlei Li rad_constituents.F90 01 ispec =',ispec
...
! Longlei Li
               write( iulog, '(1x,i2)' ) &
                     'Longlei Li rad_constituents.F90 02 ispec =',ispec

               write( iulog, '(1x,i2)' ) &
                     'Longlei Li rad_constituents.F90 02 mode_idx =',mode_idx
...
subroutine rad_cnst_get_mam_mmr_by_idx(list_idx, mode_idx, spec_idx, phase, state, pbuf, mmr)
   ...
   if (spec_idx < 1  .or.  spec_idx > modes%comps(m_idx)%nspec) then
      ...
      write(iulog,*) subname//': spec_idx= ', spec_idx, '  mode_idx= ', mode_idx, '  nspec= ', modes%comps(m_idx)%nspec
