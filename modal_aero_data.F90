...
!      integer, public, protected :: nSeaSalt=0, nDust1=0, nDust2=0,nDust3=0,nDust4=0
!      integer, public, protected ::             nDust5=0, nDust6=0,nDust7=0,nDust8=0
!#else
...
!           lptr_dust_a_amode(:),    lptr_dust_cw_amode(:)
#if ( defined DUSTTYPES )
      integer, public, protected, allocatable ::  &
          ! separate pointers for different dust types  
          lptr_dust1_a_amode(:), lptr_dust1_cw_amode(:),&   ! ptrs for mineral1
          lptr_dust2_a_amode(:), lptr_dust2_cw_amode(:),&   ! ptrs for mineral2
          lptr_dust3_a_amode(:), lptr_dust3_cw_amode(:),&   ! ptrs for mineral3
          lptr_dust4_a_amode(:), lptr_dust4_cw_amode(:),&   ! ptrs for mineral4
          lptr_dust5_a_amode(:), lptr_dust5_cw_amode(:),&   ! ptrs for mineral5
          lptr_dust6_a_amode(:), lptr_dust6_cw_amode(:),&   ! ptrs for mineral6
          lptr_dust7_a_amode(:), lptr_dust7_cw_amode(:),&   ! ptrs for mineral7
          lptr_dust8_a_amode(:), lptr_dust8_cw_amode(:)     ! ptrs for mineral8
#else
      integer, public, protected, allocatable ::  &
          lptr_dust_a_amode(:),    lptr_dust_cw_amode(:)
#endif
      ...
      integer, public, protected :: &
           ...
           modeptr_coardust,  modeptr_coarseas, modeptr_coardust1,       & 
           modeptr_coardust2, modeptr_coardust3,                         & 
           modeptr_coardust7,modeptr_coardust8,modeptr_coardust9,modeptr_coardust10 ! Longlei Li MAM10
...
!--------------------------------------------------------------
  subroutine modal_aero_data_reg
    ...
    character(len=7) :: xname_numptr, xname_numptrcw
    !character(len=1) :: modechr
    character(len=2) :: modechr
    ...
    mdiagnum_amode(:) = 0
    mprogsfc_amode(:) = 0
    if (ntot_amode==7 .or. ntot_amode==10) then ! Longlei Li MAM10
       mcalcwater_amode(:) = 1
    else
       mcalcwater_amode(:) = 0
    endif
...
#if ( defined DUSTTYPES )
         ! separate pointers for different dust types  
         lptr_dust1_a_amode(ntot_amode), lptr_dust1_cw_amode(ntot_amode),&   ! ptrs for mineral1
         lptr_dust2_a_amode(ntot_amode), lptr_dust2_cw_amode(ntot_amode),&   ! ptrs for mineral2
         lptr_dust3_a_amode(ntot_amode), lptr_dust3_cw_amode(ntot_amode),&   ! ptrs for mineral3
         lptr_dust4_a_amode(ntot_amode), lptr_dust4_cw_amode(ntot_amode),&   ! ptrs for mineral4
         lptr_dust5_a_amode(ntot_amode), lptr_dust5_cw_amode(ntot_amode),&   ! ptrs for mineral5
         lptr_dust6_a_amode(ntot_amode), lptr_dust6_cw_amode(ntot_amode),&   ! ptrs for mineral6
         lptr_dust7_a_amode(ntot_amode), lptr_dust7_cw_amode(ntot_amode),&   ! ptrs for mineral7
         lptr_dust8_a_amode(ntot_amode), lptr_dust8_cw_amode(ntot_amode),&   ! ptrs for mineral8
#else
         lptr_dust_a_amode(ntot_amode), lptr_dust_cw_amode(ntot_amode), &
#endif
...
!++Longlei Li++ for some reason I need to increase the maximum allowed species in the model
           ! to avoid unrealistically high AOD at some points for simulations after 11 model day
    nspec_max = maxval( nspec_amode )
    !nspec_max = maxval( nspec_amode ) + 3
!--
    ...
    do m = 1, ntot_amode
       do l = 1, nspec_amode(m)
          ...
          !write(modestr,'(I1)') m
          if(m.gt.9)then
              write(modestr,fmt='(I2)') m
          else
              write(modestr,fmt='(I1)') m
          end if
          idx = index( xname_massptr(l,m), '_' )
          xname_massptrcw(l,m) = xname_massptr(l,m)(:idx-1)//'_c'//modestr
!#if ( defined DUSTTYPES )
!          if (xname_massptr(l,m)(:4) == 'dst1') nDust1=nDust1+1
!          if (xname_massptr(l,m)(:4) == 'dst2') nDust2=nDust2+1
!          if (xname_massptr(l,m)(:4) == 'dst3') nDust3=nDust3+1
!          if (xname_massptr(l,m)(:4) == 'dst4') nDust4=nDust4+1
!          if (xname_massptr(l,m)(:4) == 'dst5') nDust5=nDust5+1
!          if (xname_massptr(l,m)(:4) == 'dst6') nDust6=nDust6+1
!          if (xname_massptr(l,m)(:4) == 'dst7') nDust7=nDust7+1
!          if (xname_massptr(l,m)(:4) == 'dst8') nDust8=nDust8+1
!#else
...
#if ( defined DUSTTYPES )
                case('dust-type1')
                   specmw_amode(l,m) = 389.34_r8
                case('dust-type2')
                   specmw_amode(l,m) = 258.16_r8
                case('dust-type3')
                   specmw_amode(l,m) = 549.07_r8
                case('dust-type4')
                   specmw_amode(l,m) = 159.69_r8
                case('dust-type5')
                   specmw_amode(l,m) = 60.08_r8
                case('dust-type6')
                   specmw_amode(l,m) = 100.09_r8
                case('dust-type7')
                   specmw_amode(l,m) = 278.33_r8
                case('dust-type8')
                   specmw_amode(l,m) = 172.17_r8
#else
                case('dust')
...
! Longlei Li MAM9
       ...
       modeptr_coardust7 = init_val
       modeptr_coardust8 = init_val
       modeptr_coardust9 = init_val
       modeptr_coardust10 = init_val
#endif
       do m = 1, ntot_amode
          if (modename_amode(m) .eq. 'accum') then
             ...
             modeptr_coardust = m
#elif ( defined MODAL_AERO_10MODE )
          else if (modename_amode(m) .eq. 'coarse7_dust') then
             modeptr_coardust7 = m
          else if (modename_amode(m) .eq. 'coarse8_dust') then
             modeptr_coardust8 = m
          else if (modename_amode(m) .eq. 'coarse9_dust') then
             modeptr_coardust9 = m
          else if (modename_amode(m) .eq. 'coarse10_dust') then
             modeptr_coardust10 = m
#endif
          end if
       end do
       ...
       do m = 1, ntot_amode
...
#if ( defined DUSTTYPES )
          ! keep seperate pointers for mineral species  
          lptr_dust1_a_amode(m)  = init_val
          lptr_dust1_cw_amode(m) = init_val
          lptr_dust2_a_amode(m)  = init_val
          lptr_dust2_cw_amode(m) = init_val
          lptr_dust3_a_amode(m)  = init_val
          lptr_dust3_cw_amode(m) = init_val
          lptr_dust4_a_amode(m)  = init_val
          lptr_dust4_cw_amode(m) = init_val
          lptr_dust5_a_amode(m)  = init_val
          lptr_dust5_cw_amode(m) = init_val
          lptr_dust6_a_amode(m)  = init_val
          lptr_dust6_cw_amode(m) = init_val
          lptr_dust7_a_amode(m)  = init_val
          lptr_dust7_cw_amode(m) = init_val
          lptr_dust8_a_amode(m)  = init_val
          lptr_dust8_cw_amode(m) = init_val
#else
          ...
          do l = 1, nspec_amode(m)
            ...
            ! Longlei Li, has to be 4 to get dst1 instead of dst
             tmpch3 = cnst_name(lmassa)(:4)
#if ( defined DUSTTYPES )
             !  write( iulog, '(1x,a)' ) &
             !        'tmpch3=',tmpch3
             select case (tmpch3)
             ...
             case('dst1')
                lptr_dust1_a_amode(m)  = lmassa
                lptr_dust1_cw_amode(m) = lmassc
             case('dst2')
                lptr_dust2_a_amode(m)  = lmassa
                lptr_dust2_cw_amode(m) = lmassc
             case('dst3')
                lptr_dust3_a_amode(m)  = lmassa
                lptr_dust3_cw_amode(m) = lmassc
             case('dst4')
                lptr_dust4_a_amode(m)  = lmassa
                lptr_dust4_cw_amode(m) = lmassc
             case('dst5')
                lptr_dust5_a_amode(m)  = lmassa
                lptr_dust5_cw_amode(m) = lmassc
             case('dst6')
                lptr_dust6_a_amode(m)  = lmassa
                lptr_dust6_cw_amode(m) = lmassc
             case('dst7')
                lptr_dust7_a_amode(m)  = lmassa
                lptr_dust7_cw_amode(m) = lmassc
             case('dst8')
                lptr_dust8_a_amode(m)  = lmassa
                lptr_dust8_cw_amode(m) = lmassc
             case('ncl_')
...
#elif ( defined MODAL_AERO_10MODE )
       write(iulog,*) 'modeptr_coardust7 =', modeptr_coardust7
       write(iulog,*) 'modeptr_coardust8 =', modeptr_coardust8
       write(iulog,*) 'modeptr_coardust9 =', modeptr_coardust9
       write(iulog,*) 'modeptr_coardust10 =', modeptr_coardust10
#endif
       ...
       do m = 1, ntot_amode
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust1_a_amode(m), lptr_dust1_cw_amode(m),  'dust-type1' )
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust2_a_amode(m), lptr_dust2_cw_amode(m),  'dust-type2' )
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust3_a_amode(m), lptr_dust3_cw_amode(m),  'dust-type3' )
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust4_a_amode(m), lptr_dust4_cw_amode(m),  'dust-type4' )
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust5_a_amode(m), lptr_dust5_cw_amode(m),  'dust-type5' )
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust6_a_amode(m), lptr_dust6_cw_amode(m),  'dust-type6' )
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust7_a_amode(m), lptr_dust7_cw_amode(m),  'dust-type7' )
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_dust8_a_amode(m), lptr_dust8_cw_amode(m),  'dust-type8' )
       end do
#else
       do m = 1, ntot_amode
          call initaermodes_setspecptrs_write2( m,                    &
