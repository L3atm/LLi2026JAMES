...
integer :: mode_fine_dst_idx = -1  ! index of fine dust mode
...
#if ( defined DUSTTYPES )
integer :: cdst1_idx, cdst2_idx, cdst3_idx, cdst4_idx, cdst5_idx, cdst6_idx,cdst7_idx, cdst8_idx
#endif
...
subroutine nucleate_ice_cam_init(mincld_in, bulk_scale_in)
 ...
 use cam_logfile,     only: iulog
...
#if ( defined DUSTTYPES )
   ! fractional occurance of mixed-phase clouds
   call addfld('FREQMEY',(/ 'lev' /), 'A', 'fraction ', 'Fractional occurance of mixed-phase clouds')

   ! IN concentration as diagnostic at serveral temperatures
   call addfld ('CIN1    ', (/ 'lev' /), 'A', '1/cm3   ', 'IN concentration at T=-12C')
   call addfld ('CIN2    ', (/ 'lev' /), 'A', '1/cm3   ', 'IN concentration at T=-16C')
   call addfld ('CIN3    ', (/ 'lev' /), 'A', '1/cm3   ', 'IN concentration at T=-20C')
   call addfld ('CIN4    ', (/ 'lev' /), 'A', '1/cm3   ', 'IN concentration at T=-24C')
   call addfld ('CIN5    ', (/ 'lev' /), 'A', '1/cm3   ', 'IN concentration at T=-28C')
   call addfld ('CIN6    ', (/ 'lev' /), 'A', '1/cm3   ', 'IN concentration at T=-32C')
#endif
   ...
   if (clim_modal_aero) then
      ...
      ! for 3-mode 
      if (mode_coarse_dst_idx < 0) mode_coarse_dst_idx = mode_coarse_dst_idx
...
#if ( defined DUSTTYPES )
      if ( coarse_dust1_idx == -1 .or. coarse_dust2_idx == -1 .or. coarse_dust3_idx == -1 .or. coarse_dust4_idx == -1 .or. &
           coarse_dust5_idx == -1 .or. coarse_dust6_idx == -1 .or. coarse_dust7_idx == -1 .or. coarse_dust8_idx == -1 .or. &
            coarse_nacl_idx == -1 .or.     accum_bc_idx == -1 .or.  accum_dust1_idx == -1 .or.  accum_dust2_idx == -1 .or. &
            accum_dust3_idx == -1 .or.  accum_dust4_idx == -1 .or.  accum_dust5_idx == -1 .or.  accum_dust6_idx == -1 .or. &
            accum_dust7_idx == -1 .or.  accum_dust8_idx == -1 ) then
         write(iulog,*) routine//': ERROR required mode-species type not found - indicies:', &
            ...
            coarse_dust5_idx, coarse_dust6_idx, coarse_dust7_idx, coarse_dust8_idx, &
             coarse_nacl_idx,     accum_bc_idx,  accum_dust1_idx,  accum_dust2_idx, &
             accum_dust3_idx,  accum_dust4_idx,  accum_dust5_idx,  accum_dust6_idx, &
             accum_dust7_idx,  accum_dust8_idx
...
#if ( defined DUSTTYPES )
!    write(iulog,*) 'Longlei Li marks begin in nucleate_ice_cam: test01'

         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust1_idx, cdst1_idx)

!    write(iulog,*) 'Longlei Li marks begin in nucleate_ice_cam: test02'

         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust2_idx, cdst2_idx)
         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust3_idx, cdst3_idx)
         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust4_idx, cdst4_idx)
         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust5_idx, cdst5_idx)
         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust6_idx, cdst6_idx)
         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust7_idx, cdst7_idx)
         call rad_cnst_get_mam_mmr_idx(mode_coarse_dst_idx, coarse_dust8_idx, cdst8_idx)
#else
...
use cam_logfile,       only: iulog
...
#if ( defined DUSTTYPES )   
   real(r8), pointer :: accum_a_bc(:,:)
   real(r8), pointer :: accum_a_dust1(:,:)
   real(r8), pointer :: accum_a_dust2(:,:)
   real(r8), pointer :: accum_a_dust3(:,:)
   real(r8), pointer :: accum_a_dust4(:,:)
   real(r8), pointer :: accum_a_dust5(:,:)
   real(r8), pointer :: accum_a_dust6(:,:)
   real(r8), pointer :: accum_a_dust7(:,:)
   real(r8), pointer :: accum_a_dust8(:,:)

   real(r8), pointer :: coarse_a_dust1(:,:)
   real(r8), pointer :: coarse_a_dust2(:,:)
   real(r8), pointer :: coarse_a_dust3(:,:)
   real(r8), pointer :: coarse_a_dust4(:,:)
   real(r8), pointer :: coarse_a_dust5(:,:)
   real(r8), pointer :: coarse_a_dust6(:,:)
   real(r8), pointer :: coarse_a_dust7(:,:)
   real(r8), pointer :: coarse_a_dust8(:,:)
#else
...
#if ( defined DUSTTYPES )
   real(r8), pointer :: cld_coarse_dust1(:,:)
   real(r8), pointer :: cld_coarse_dust2(:,:)
   real(r8), pointer :: cld_coarse_dust3(:,:)
   real(r8), pointer :: cld_coarse_dust4(:,:)
   real(r8), pointer :: cld_coarse_dust5(:,:)
   real(r8), pointer :: cld_coarse_dust6(:,:)
   real(r8), pointer :: cld_coarse_dust7(:,:)
   real(r8), pointer :: cld_coarse_dust8(:,:)
#else
...
#if ( defined DUSTTYPES )
   integer,     parameter ::  ptem = 6
   real(r8),    parameter ::  supertemk(ptem)=&               ! supercooling (K) to determine IN concentration
                              (/261.15, 257.15, 253.15, 249.15, 245.15, 241.15/)
   real(r8) :: cin(pcols,pver,ptem)                       ! number conc of aerosols activated at supertem   
   character*8, parameter :: cin_name(ptem)=(/'CIN1','CIN2','CIN3','CIN4','CIN5','CIN6'/)
   integer  :: l
   ...
   ! Atkinson et al. for mixed-phase cloud immersion ice nucleation (surface
   ! area based, K-feldspar)
   logical, parameter :: atk_in = .true.
   real(r8) at
   real(r8) k_fsp
   real(r8) ns_dust_imm    ! dust active site surface densities (m-2)
#endif

   !-------------------------------------------------------------------------------
    !write(iulog,*) 'Longlei marks begin in nucleate_ice_cam'
   lchnk = state%lchnk
   ncol  = state%ncol
   t     => state%t
   qn    => state%q(:,:,1)
...
#if ( defined DUSTTYPES )
      call rad_cnst_get_aer_mmr(0, mode_accum_idx,    accum_bc_idx, 'a', state, pbuf, accum_a_bc)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust1_idx, 'a', state, pbuf, accum_a_dust1)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust2_idx, 'a', state, pbuf, accum_a_dust2)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust3_idx, 'a', state, pbuf, accum_a_dust3)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust4_idx, 'a', state, pbuf, accum_a_dust4)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust5_idx, 'a', state, pbuf, accum_a_dust5)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust6_idx, 'a', state, pbuf, accum_a_dust6)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust7_idx, 'a', state, pbuf, accum_a_dust7)
      !call rad_cnst_get_aer_mmr(0, mode_accum_idx, accum_dust8_idx, 'a', state, pbuf, accum_a_dust8)

      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust1_idx, 'a', state, pbuf, accum_a_dust1)
      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust2_idx, 'a', state, pbuf, accum_a_dust2)
      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust3_idx, 'a', state, pbuf, accum_a_dust3)
      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust4_idx, 'a', state, pbuf, accum_a_dust4)
      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust5_idx, 'a', state, pbuf, accum_a_dust5)
      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust6_idx, 'a', state, pbuf, accum_a_dust6)
      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust7_idx, 'a', state, pbuf, accum_a_dust7)
      call rad_cnst_get_aer_mmr(0, mode_fine_dst_idx, accum_dust8_idx, 'a', state, pbuf, accum_a_dust8)

    !write(iulog,*) 'Longlei Li marks begin in nucleate_ice_cam: test07'

      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust1_idx, 'a', state, pbuf, coarse_a_dust1)

    !write(iulog,*) 'Longlei Li marks begin in nucleate_ice_cam: test08'

      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust2_idx, 'a', state, pbuf, coarse_a_dust2)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust3_idx, 'a', state, pbuf, coarse_a_dust3)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust4_idx, 'a', state, pbuf, coarse_a_dust4)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust5_idx, 'a', state, pbuf, coarse_a_dust5)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust6_idx, 'a', state, pbuf, coarse_a_dust6)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust7_idx, 'a', state, pbuf, coarse_a_dust7)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust8_idx, 'a', state, pbuf, coarse_a_dust8)

    !write(iulog,*) 'Longlei Li marks begin in nucleate_ice_cam: test09'
...
#if ( defined DUSTTYPES )
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust1_idx, 'c', state, pbuf, cld_coarse_dust1)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust2_idx, 'c', state, pbuf, cld_coarse_dust2)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust3_idx, 'c', state, pbuf, cld_coarse_dust3)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust4_idx, 'c', state, pbuf, cld_coarse_dust4)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust5_idx, 'c', state, pbuf, cld_coarse_dust5)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust6_idx, 'c', state, pbuf, cld_coarse_dust6)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust7_idx, 'c', state, pbuf, cld_coarse_dust7)
      call rad_cnst_get_aer_mmr(0, mode_coarse_dst_idx, coarse_dust8_idx, 'c', state, pbuf, cld_coarse_dust8)
#else
...
#if ( defined DUSTTYPES )
   freqmey(1:ncol,1:pver) = 0._r8   ! iceMP
   ...
   do k = top_lev, pver
      do i = 1, ncol
         ...
         if (t(i,k) < tmelt - 5._r8) then
            ...
            if (clim_modal_aero) then
#if ( defined DUSTTYPES )
               soot_num = accum_a_bc(i,k)*soot_num_to_mass*1.0e-6_r8  ! #/cm^3

               soot_sfc = accum_a_bc(i,k)*soot_sfc_to_mass*1.0e-6_r8  ! m2/cm^3

               dmc = accum_a_dust1(i,k) + accum_a_dust2(i,k) + accum_a_dust3(i,k) + accum_a_dust4(i,k) + &
                     accum_a_dust5(i,k) + accum_a_dust6(i,k) + accum_a_dust7(i,k) + accum_a_dust8(i,k)

               dst1_num = dmc * dst1_num_to_mass * 1.0e-6_r8

               dst1_sfc = dmc * dst1_sfc_to_mass * 1.0e-6_r8

               x_fsp_accum = accum_a_dust7(i,k)/dmc

               dmc = coarse_a_dust1(i,k) + coarse_a_dust2(i,k) + coarse_a_dust3(i,k) + coarse_a_dust4(i,k) + &
                     coarse_a_dust5(i,k) + coarse_a_dust6(i,k) + coarse_a_dust7(i,k) + coarse_a_dust8(i,k)

               x_fsp_coarse = coarse_a_dust7(i,k)/dmc

               ssmc = coarse_nacl(i,k)

               if ( dmc > 0._r8 ) then
                  dst3_num = dmc/(dmc+ssmc)*num_coarse(i,k)*rho(i,k)*1.0e-6_r8
               else
                  dst3_num = 0.0_r8
               endif
               ...
               if ( dmc > 0._r8 ) then
                  dst3_sfc = dmc/(dmc+ssmc)*pi*dgnum(i,k,mode_coarse_dst_idx)**2.0_r8*exp(2.0_r8*alnsg**2.0_r8)&
                             *num_coarse(i,k)*rho(i,k)*1.0e-6_r8      ! m2/cm^3
               else
                  dst3_sfc = 0.0_r8
               endif

               dst3_sfc_to_num = pi*dgnum(i,k,mode_coarse_dst_idx)**2.0_r8*exp(2.0_r8*alnsg**2.0_r8) ! m2/#, individual particle sfc

               dst_num = dst1_num + dst3_num    ! dust number (#/cm^3) with D>0.1 um
               dst_sfc = dst1_sfc + dst3_sfc    ! dust area (m2/cm^3) with D>0.1 um
#else
            ...
            call nucleati( &
               ...
               dst1_num, dst3_num, dst1_sfc_to_num, dst3_sfc_to_num,     &
               x_fsp_accum, x_fsp_coarse,                                &
               naai(i,k), nihf(i,k), niimm(i,k), nidep(i,k), nimey(i,k), &
               wice(i,k), weff(i,k), fhom(i,k), regm(i,k), freqmey(i,k), &
...
#if ( defined DUSTTYPES )
! too high AOD
!               ptend%q(i,k,cnum_idx) = -(odst1_num * icldm(i,k))/rho(i,k)/1e-6_r8/dtime
!               ptend%q(i,k,cnum_idx) = -(odst3_num * icldm(i,k))/rho(i,k)/1e-6_r8/dtime
!               cld_num_coarse(i,k)   = cld_num_coarse(i,k) + (odst1_num * icldm(i,k))/rho(i,k)/1e-6_r8
!               cld_num_coarse(i,k)   = cld_num_coarse(i,k) + (odst3_num * icldm(i,k))/rho(i,k)/1e-6_r8

               ptend%q(i,k,cdst1_idx) = - odst1_num / dst1_num * icldm(i,k) * coarse_a_dust1(i,k) / dtime
               ptend%q(i,k,cdst3_idx) = - odst3_num / dst3_num * icldm(i,k) * coarse_a_dust3(i,k) / dtime

               cld_coarse_dust1(i,k) = cld_coarse_dust1(i,k) + odst1_num / dst1_num *icldm(i,k) * coarse_a_dust1(i,k)
               cld_coarse_dust3(i,k) = cld_coarse_dust3(i,k) + odst3_num / dst3_num *icldm(i,k) * coarse_a_dust3(i,k)
#else
!               ptend%q(i,k,cnum_idx) = -(odst_num * icldm(i,k))/rho(i,k)/1e-6_r8/dtime
!               cld_num_coarse(i,k)   = cld_num_coarse(i,k) + (odst_num * icldm(i,k))/rho(i,k)/1e-6_r8
...
!#if ( defined DUSTTYPES )
!               INndust(i,k)=dst3_num*1e6_r8
!               INondust(i,k)=odst3_num*1e6_r8
!#else
...
!#endif
         ...
         do i = 1, ncol
            ...
            if (clim_modal_aero) then
               ...
               if (atk_in) then
                  ...
                  at = -1.038_r8*max(248.0_r8,supertemk(l)) + 275.26_r8    ! valid between 248 and 268 K
                  k_fsp = 0.35_r8                                          ! mass fraction of K-feldspar in total feldspar
                  ns_dust_imm = exp(at)*1.e+4_r8                           ! m^-2, valid for 248 to 268 K

                  cin(i,k,l) = dst1_num * (1._r8 - exp(-1.0_r8 * dst1_sfc_to_num * ns_dust_imm * x_fsp_accum * k_fsp)) &   ! cm^-3
                              +dst3_num * (1._r8 - exp(-1.0_r8 * dst3_sfc_to_num * ns_dust_imm * x_fsp_coarse * k_fsp))
   ...
   do l = 1, ptem
     call outfld (cin_name(l), cin(1:pcols,1:pver,l), pcols, lchnk)
   end do
#endif
...
#if ( defined DUSTTYPES )
   call outfld('FREQMEY',freqmey, pcols, lchnk)    ! iceMP
#endif
   ...
   if (use_preexisting_ice) then
    ...
    !write(iulog,*) 'Longlei marks end in nucleate_ice_cam'
